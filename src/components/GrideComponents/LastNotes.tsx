...
export default function notes() {
  ...
  <div className="flex h-full min-h-0 flex-col rounded-xl border border-gray-200 bg-white transition-colors dark:border-[#303030] dark:bg-[#252526]">
    ...
    <div className="flex shrink-0 items-center gap-2 px-8 py-5">
      ...
      <h2 className="font-semibold text-gray-900 dark:text-[#cccccc]">Ultimi appunti</h2>
    </div>
    ...
    <div className="custom-scrollbar min-h-0 flex-1 overflow-y-auto divide-y divide-gray-100 dark:divide-[#303030]">
      {files.map((note) => {
        ...
        return (
          <div key={note.id} className="flex items-center gap-4 px-4 py-3 transition-colors hover:bg-gray-50 dark:hover:bg-[#2a2d2e]">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg" style={{ backgroundColor: color }}>
            ...
            <span className="hidden rounded-md px-2 py-1 text-xs font-medium text-white sm:block" style={{ backgroundColor: color }}>
              {note.course}
            </span>

            <span className="text-xs text-gray-500 dark:text-[#9d9d9d]">
              {note.created_at ? new Date(note.created_at).toLocaleDateString("it-IT") : "Nessuna data"}
            </span>
          </div>
        );
      })}
