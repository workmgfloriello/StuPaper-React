...
export default function RecentCourses() {
  ...
  const recentCourses = courses
    ...
    const handleCourseClick = (e: React.MouseEvent<HTMLDivElement, MouseEvent>) =>{
      const courseId = e.currentTarget.dataset.ref
      navigator(`/corsi/${courseId}`)
    }
  return (
  <div className="flex h-full min-h-0 flex-col rounded-xl border border-gray-300 bg-white p-5 transition-colors dark:border-[#303030] dark:bg-[#252526]">
    ...
    <div className="mt-4 grid min-h-0 flex-1 grid-cols-1 gap-4 sm:grid-cols-3">
      {recentCourses.map((course) => (
        <div onClick={(e)=>handleCourseClick(e)} data-ref={course.id} key={course.id} className="flex min-h-0 min-w-0 flex-col justify-between rounded-xl border border-gray-200 bg-white p-4 shadow-sm transition hover:-translate-y-1 hover:shadow-md dark:border-[#303030] dark:bg-[#1e1e1e] dark:shadow-none dark:hover:border-[#454545] dark:hover:bg-[#2a2d2e]">
          {/* Card header */}
